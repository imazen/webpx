...
fn main() {
    ...
    eprintln!("Image: {} ({}x{}, {} pixels)", path.display(), width, height, pixels);
    ...
    eprintln!("  min: {:.2} MB", est.peak_memory_bytes_min as f64 / 1_000_000.0);
    eprintln!("  typ: {:.2} MB", est.peak_memory_bytes as f64 / 1_000_000.0);
    eprintln!("  max: {:.2} MB", est.peak_memory_bytes_max as f64 / 1_000_000.0);
    ...
    eprintln!("Output: {} bytes ({:.2}% of input)", 
        result.len(), 
